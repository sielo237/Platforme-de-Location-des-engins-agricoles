...
import { getConstencyRatio } from "../../../AHP/ahp";
...
const RecommendationPage = () => {
  ...
  const handleNext = () => {
    if(step === 1){
        ...
        var newMatrix = completeArray();
        getConstencyRatio(newMatrix)
    }
  }
      ...
      <Container>
        ...
        <div className="grid grid-cols-5 gap-8 mt-4">
          <div className=" bg-white rounded-md col-span-3 p-2">
           ...
           <button className={` ${ step===1 &&"hidden"} mx-2 rounded-md text-secondary border border-secondary py-1.5 px-4 bg-white`} onClick={() => handlePrev()} >Précedent</button>
            <button className="mx-2 rounded-md border border-secondary py-1.5 px-4 bg-secondary text-white" onClick={() => handleNext()}>{step<3 ? "Suivant":"Recommender"}</button>
           </div>
          </div>
